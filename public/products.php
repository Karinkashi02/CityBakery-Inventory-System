...
if (isset($_GET['search']) && !empty(trim($_GET['search']))) {
    ...
    $sql = "SELECT p.*, c.CAT_NAME AS CAT_NAME, s.SUP_NAME AS SUP_NAME
            FROM PRODUCT p
            LEFT JOIN CATEGORY c ON p.CAT_ID = c.CAT_ID
            LEFT JOIN SUPPLIER s ON p.SUP_ID = s.SUP_ID
            WHERE LOWER(p.PROD_NAME) LIKE '%' || :search || '%'
            OR LOWER(c.CAT_NAME) LIKE '%' || :search || '%'
            OR LOWER(s.SUP_NAME) LIKE '%' || :search || '%'";
...
<body>
    ...
    <section class="home">
        <button class="addButton" onclick="location.href='add_prod.php'">
            <b>ADD</b>
        </button>
        ...
        <div class="container">
            ...
            <table>
                <thead>
                    <tr>
                        <th>Name</th>
                        <th>Buy Price (1 Unit)</th>
                        <th>Sell Price (1 Unit)</th>
                        <th>Quantity</th>
                        <th>SUPPLIER NAME</th>
                        <th>Category</th>
                        <th>Actions</th>
                ...
                <tbody overflow-y: auto>
                    <?php 
                        ...
                        while ($row = oci_fetch_assoc($stmt)) {
                            ...
                            echo "<td>RM" . number_format((float)$row['PROD_BUY'], 2) . "</td>";
                            echo "<td>RM" . number_format((float)$row['PROD_SALE'], 2) . "</td>";
                            echo "<td>" . htmlspecialchars($row['PROD_QTY']) . "</td>";
                            $supName = isset($row['SUP_NAME']) && trim($row['SUP_NAME']) !== '' ? htmlspecialchars($row['SUP_NAME']) : 'N/A';
                            echo "<td>" . $supName . "</td>";
                            echo "<td>" . htmlspecialchars($row['CAT_NAME']) . "</td>";
                            echo "<td>
                                <a href='edit_prod.php?id=" . urlencode($row['PROD_ID']) . "'><i class='bx bx-edit-alt icon'></i></a>
                                <a href='del_prod.php?id=" . urlencode($row['PROD_ID']) . "' onclick=\"return confirm('Are you sure you want to delete this product?');\"><i class='bx bx-trash icon'></i></a>
                                </td>";
                            echo "</tr>";
                        }
