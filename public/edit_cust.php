...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['update_customer'])) {
    ...
    $update_sql = "UPDATE CUSTOMER SET CUST_NAME = :name WHERE CUST_ID = :id";
    $update_stmt = oci_parse($conn, $update_sql);

    oci_bind_by_name($update_stmt, ':name', $name);
    oci_bind_by_name($update_stmt, ':id', $customer_id);

    if (oci_execute($update_stmt)) {
        $_SESSION['msg'] = ['s', 'Customer updated successfully.'];
...
if (!$customer) {
    $_SESSION['msg'] = ['d', 'Customer not found.'];
    header("Location: customer.php");
    exit();
}
?>
...
<nav class="sidebar close">
    ...
    <section class="home">
        <div class="container">
            ...
            <form method="post" action="edit_cust.php?id=<?php echo (int)$customer['CUST_ID']; ?>">
                <h2 style="text-align: center;"><u>EDIT CUSTOMER</u></h2><br>
                <div class="form-group">
                    <label for="name">Customer Name</label>
                    <input type="text" class="form-control" name="name" value="<?php echo htmlspecialchars($customer['CUST_NAME']); ?>" required>
                </div>
                <br>
